-- Spring Security 7.1.1 JDBC schemas, adapted to PostgreSQL text/timestamptz.
...
ALTER TABLE oauth2_registered_client ADD CONSTRAINT uk_oauth2_client_id UNIQUE (client_id);
ALTER TABLE oauth2_authorization ADD CONSTRAINT fk_authorization_client
...
CREATE INDEX idx_authorization_state ON oauth2_authorization (state);
-- Token values are potentially larger than PostgreSQL btree index entries.
CREATE INDEX idx_authorization_code ON oauth2_authorization USING hash (authorization_code_value);
CREATE INDEX idx_authorization_access_token ON oauth2_authorization USING hash (access_token_value);
CREATE INDEX idx_authorization_refresh_token ON oauth2_authorization USING hash (refresh_token_value);
CREATE INDEX idx_authorization_id_token ON oauth2_authorization USING hash (oidc_id_token_value);
GRANT SELECT ON oauth2_registered_client TO "${runtimeRole}";
GRANT SELECT, INSERT, UPDATE, DELETE ON oauth2_authorization, oauth2_authorization_consent TO "${runtimeRole}";
